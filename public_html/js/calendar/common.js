function addEvent(elm, event, fn, capture)
{
	JSL.dom(elm).on(event, fn);
}
function findTarget(e)
{
	return JSL.event(e).getTarget();
}
function getElementsByCSS(selector)
{
	return JSL.dom(selector).get();
}
...
{
	if(state)item.style.display = state;
	else item.style.display = (item.style.display == "block") ? "none" : "block";
}
function stopEvent(e)
{
	return JSL.event(e).stop();
}
