...
if ($variable->post('add-action', 'string', "")) {
    ...
    if ($crud_id = $permissions->add($data, false)) {
        # Send a welcome message (and ask to authenticate)
        #$permissions->welcome_first($crud_id);
        ...
        #\common\stopper::url('permissions-add-successful.php');
        #\common\stopper::url('permissions-list.php');

        # Once successfully added, as the admin user to edit the data.
        #\common\stopper::url('permissions-edit.php?id='.$crud_id);
    } else {
        ...
        \common\stopper::url('permissions-add-error.php');
    }
} else {
    ...
    $details = $permissions->validate('add', $details);
    $smarty->assign('permissions', $details);
}

$smarty->assign('protection_code', $permissions->code());
