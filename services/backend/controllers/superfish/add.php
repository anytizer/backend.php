...
if($variable->post('add-action', 'string', ''))
{
	...
	if($menu_id = $superfish->add($data, false))
	{
		...
		#\common\stopper::url('superfish-add-successful.php');
		#\common\stopper::url('superfish-list.php');

		# Once successfully added, as the admin user to edit the data.
		#\common\stopper::url('superfish-edit.php?id='.$menu_id);
	}
	else
	{
		...
		\common\stopper::url('superfish-add-error.php');
	}
}
else
{
	...
	$details = $superfish->validate('add', $details);
	$smarty->assign('superfish', $details);
}

$smarty->assign('protection_code', $superfish->code());
