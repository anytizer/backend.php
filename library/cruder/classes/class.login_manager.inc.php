    ...
    extends \common\mysql
{
    ...
    public function login_user($username = "", $password = "")
    {
        ...
        $password = \common\tools::encrypt($password);
...
SELECT
	...
	`user_password`, # Some apps may externally verify the password
...
# Degraded SQL for compatibility in CentOS like linux test servers
# COUNT() should be used with GROUP BY only
GROUP BY
	`u`.`user_name`
;";
    ...
    public function logout_user($user_id = 0)
    {
        ...
        foreach ($kills as $kill) {
            $variable->write('session', $kill, false);
            $variable->kill($kill);
        }
