 ...
 * @link http://sudocode.net/sources/includes/class-xhttp-php/plugin-xhttp-oauth-php/
 */
class xhttp_oauth
{
	...
	public static function oauth($profile, $consumer_key, $consumer_secret = null, $token = null, $token_secret = null)
	{
	...
	{
		xhttp::load('profile');
		xhttp::addHook('data-preparation', array(__CLASS__, 'oauth_sign_request'), 8);
	...
	public static function oauth_sign_request(&$urlparts, &$requestData)
	{
		...
		{
			foreach($requestData['get'] as $key => $value) $oauth_data[$key] = $value;
		}
		if(isset($requestData['post']) and is_array($requestData['post']))
		{
			foreach($requestData['post'] as $key => $value) $oauth_data[$key] = $value;
		}
		...
		// Create base string
		$array = array();
		foreach($parameters as $index => $pair) if($pair[1] !== null)
		{
			$array[] = $pair[0] . '=' . $pair[1];
		}
		...
		if(isset(self::$datastore[$profile]['method']) && (self::$datastore[$profile]['method'] == 'get' or self::$datastore[$profile]['method'] == 'post'))
		{
		...
		{
			$vars = array();
			foreach($requestData['get'] as $key => $value) if(substr($key, 0, 5) != 'oauth' or self::$datastore[$profile]['method'] == 'get')
			{
				$vars[$key] = $value;
			}
		...
		{
			$vars = array();
			foreach($requestData['post'] as $key => $value) if(substr($key, 0, 5) != 'oauth' or self::$datastore[$profile]['method'] == 'post')
			{
				$vars[$key] = $value;
			}
	...
	{
		return str_replace('%7E', '~', rawurlencode(utf8_encode($string)));
	}
