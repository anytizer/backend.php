 ...
 * $dumper = new \common\mysqlDump($dbname,'filename.sql',false,false);
 * $dumper->doDump();
 * </code>
 *
 * @package Databases
...
class MySQLDump
{
	...
	function doDump()
	{
		$this->saveToFile($this->file, $this->filehead . "SET FOREIGN_KEY_CHECKS = 0;\n\n");
	...
	function getSqlKeysTable($table)
	{
		...
		while($row = mysql_fetch_object($results))
		{
			...
			{
				if((!isset($unique)) OR (!is_array($unique)) OR ($unique[$row->Key_name] == ""))
				{
